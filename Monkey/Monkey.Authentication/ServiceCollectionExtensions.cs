    ...
    public static class ServiceCollectionExtensions
    {
        ...
        public static void BuildConfig(this IConfiguration configuration, string configSection = Constants.DefaultConfigSection)
        {
            ...
            if (isHaveConfig)
            {
                ...
                AuthenticationConfig.SystemClientId = configuration.GetValue($"{configSection}:{nameof(AuthenticationConfig.SystemClientId)}", AuthenticationConfig.SystemClientId);
                AuthenticationConfig.SystemClientSecret = configuration.GetValue($"{configSection}:{nameof(AuthenticationConfig.SystemClientSecret)}", AuthenticationConfig.SystemClientSecret);
            }
        }
    }
}